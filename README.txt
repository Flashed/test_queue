...
P.S.: Демон читает сообщения в каталог /tmp/messages. Его можно запустиь после ./write_mess.sh, чтобы увидеть как работает подкачка.
      Модуль ядра создает два символьных устройсва: queue_pop- для чтения сообщений и queue_push -для записи.  Cохраняет сообщения, которые не поместились в очередь в каталог /tmp. Удаляет после подкачки. Если сообщение больше 64K, оно не принимается. Пишется сообщение об ошибки в лог (dmesg). 
